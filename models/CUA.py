...
class CUAModel(nn.Module):
    ...
    def forward(self, s_inputs, s_outputs, t_inputs, alpha):
        ...
        s_feats = all_s_feats[-s_inputs[0].shape[0]:]
        domain_preds = self.discriminator(ReverseLayerF.apply(torch.cat([s_feats, t_feats], dim=0), alpha))
        domain_labels = np.array([0] * s_inputs[0].shape[0] + [1] * t_inputs.shape[0])
